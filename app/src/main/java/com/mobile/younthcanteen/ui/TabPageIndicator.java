...
public class TabPageIndicator extends HorizontalScrollView {
    ...
    private int indicatorColor = Color.parseColor("#ffd161");//指示线的颜色  #0075E7
    ...
    private int indicatorHeight = 8;//指示线的高度
    private int underlineHeight = 0;// 默认指示线的高度
    ...
    private int tabTextColor = getResources().getColor(R.color.grayblack);// 标题未被选中时字体颜色
    private int tabTextColorSelected = getResources().getColor(R.color.black);// 标题被选中时字体颜色
...
/**
 * 临时替换成wrapcontent，用来测量标题宽度，进而设置下划线长度
 */
                ViewGroup.LayoutParams old = tab.getLayoutParams();
                tab.setLayoutParams(wrapTabLayoutParams);
                int widthSpec = MeasureSpec.makeMeasureSpec(1000, MeasureSpec.AT_MOST);
                int heightSpec = MeasureSpec.makeMeasureSpec(1000, MeasureSpec.AT_MOST); ;
                tab.measure(widthSpec,heightSpec);
                widths[i] = tab.getMeasuredWidth();
                tab.setLayoutParams(old);
            }
        }
    }
...
//        MyLog.e("daimin","lineLeft =" + lineLeft +"lineRight ="+lineRight);
//        MyLog.e("daimin","currentOffWid=" + currentOffWid);
